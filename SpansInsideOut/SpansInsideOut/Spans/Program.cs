...
manyNumbers.ForEach(numbersMemory =>
{
    ...
    BubbleSort3(localSpan);
    Print3(localSpan);
});

#region Sort algorithms
static void BubbleSort1<T>(T[] list) where T : IComparable
{
    bool madeChanges;
...
static void BubbleSort2<T>(MySpan<T> list) where T : IComparable
{
    bool madeChanges;
    int itemCount = list.Length;
    do
...
static void BubbleSort3<T>(Span<T> list) where T : IComparable
{
    bool madeChanges;
    int itemCount = list.Length;
    do
...
static void Print2<T>(MySpan<T> list) => Print(list.ToArray());
static void Print3<T>(Span<T> list) => Print(list.ToArray());
#endregion

...
export function printLinkedPersons(accountNumber: string): void {
  ...
  const getAccount = db.prepare<string, { id: number }>('SELECT id FROM Account WHERE number = ?');
  const getDirectHolders = db.prepare<number, { id: number; name: string }>(`
        SELECT Person.id, Person.name 
        FROM Holder 
        JOIN Person ON Holder.person_id = Person.id 
        WHERE Holder.account_id = ?
    `);
  const getConnections = db.prepare<number, { person2_id: number }>(`
        SELECT person2_id 
        FROM Connection 
        WHERE person1_id = ?
    `);
  ...
  function findConnectedPersons(personId: number) {
    if (visitedPersons.has(personId)) return; // Prevent processing the same person again
    ...
    const person = db.prepare<number, { name: string }>('SELECT name FROM Person WHERE id = ?').get(personId);
    if (person) {
